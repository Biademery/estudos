...
const getTodos = (url, callback) => {
  ...
  request.addEventListener('readystatechange', () => {
    ...
    if (isRequestNotOk) {
      callback('Não foi possível obter os dadors da API', null);
    }
  });
...
getTodos('https://pokeapi.co/api/v2/pokemon/1', (error, data) => {
  logPokemonData(error, data);
  getTodos('https://pokeapi.co/api/v2/pokemon/4', (error, data) => {
    logPokemonData(error, data);
    getTodos('https://pokeapi.co/api/v2/pokemon/7', logPokemonData);
  });
});
