...
const audiA8 = Object.create.apply(carProto);
const volvoS90 = Object.create.apply(carProto);
...
const makeExtendedClock = ({ precision = 1000 }) => ({
  precision,
  ...makeClock({ template }),
  start() {
    this.render();
...
const getStringWithCommas = ({ cells }) =>
  Array.from(cells).map(getStringWithCommas).join('\n');
...
const currencyOneEl = document.querySelector('[data-js="currecy-one"');
const currencyTwoEl = document.querySelector('[data-js="currecy-two"');
const currenciesEl = document.querySelector('[data-js="currecies-container"');

const url =
  'https://v6.exchangerate-api.com/v6/c41def264282ca29bba39350/latest/kkk';

const getErrormessage = errorType =>
  ({
    'unsuported-code': 'A moeda não existe em nosso banco de dados',
    'base-code-only-on-pro':
      'Informações de moedas que não sejam USD ou EUR só podem ser acessadas a partir',
  }[errorType]);
...
const fetchExchangeRate = async () => {
  try {
    ...
    if (!response.ok) {
      throw new Error('Sua conexão falhou. Não foi possível obter informções.');
    }
