...
const usersGithub = async username => {
  const response = await fetch(`https://api.github.com/users/${username}`);
  return response.json();
};

const logUsers = async username => {
  const users = await usersGithub(username);
  console.log(users);
};
...
const myName = ['Be', 'a', 'triz'];
const namePLanguage = myName.reduce(
